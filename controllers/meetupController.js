...
class MeetupController {
  async getAll(req, res) {
    ...
    async function getMeetups(title, keywords, limit = 5, page = 1, sort = 0) {
      ...
      if (!title && !keywords) {
        if (sort) {
          return (meetups = await meetupDB.findAll({
            order: [["id", "ASC"]],
            limit,
            offset,
          }));
  ...
  async create(req, res) {
    try {
      ...
      return res
        .status(400)
        .send(
          "The paremetrs are set incorrectly. All parametrs must be a string."
        );
    }

    const { title, description, keywords, eventInformation } = req.body;
    const meetup = await meetupDB.create({
      title,
      description,
      keywords,
      eventInformation,
  ...
  async update(req, res) {
    ...
    await meetupDB.update(
      { title, description, keywords, eventInformation },
      { where: { id } }
    );
